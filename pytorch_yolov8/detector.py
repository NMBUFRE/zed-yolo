...
# # Run inference
# results = model.predict(img, conf=0.4, device="cuda")
...
def detections_to_custom_box(detections, im0, model):
    ...
    with open('class_names.yaml', 'w') as f:
    ...
    for i, det in enumerate(detections):
        ...
        class_id = int(det.cls[0]) if hasattr(det.cls, "__getitem__") else int(det.cls)
        obj.label = class_id  # ZED needs this to be an int
        obj.probability = det.conf[0] if hasattr(det.conf, "__getitem__") else det.conf
        obj.is_grounded = False
        output.append(obj)
...
def torch_thread(weights, img_size, conf_thres=0.2, iou_thres=0.45):
    ...
    while not exit_signal:
        if run_signal:
            ...
            img = cv2.cvtColor(image_net, cv2.COLOR_RGBA2RGB)
            # https://docs.ultralytics.com/modes/predict/#video-suffixes
            det = model.predict(img, save=False, imgsz=img_size, conf=conf_thres, iou=iou_thres, device="cuda")[0].cpu().numpy().boxes
...
def main():
    ...
    while viewer.is_available() and not exit_signal:
        if zed.grab(runtime_params) == sl.ERROR_CODE.SUCCESS:
            ...
            # 2D rendering
            np.copyto(image_left_ocv, image_left.get_data())
            cv_viewer.render_2D(image_left_ocv, image_scale, objects, obj_param.enable_tracking, names=model_names)
            global_image = cv2.hconcat([image_left_ocv, image_track_ocv])
            # Tracking view
            track_view_generator.generate_view(objects, cam_w_pose, image_track_ocv, objects.is_tracked)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--weights', type=str, default='best_colab_train.pt', help='model.pt path(s)')
    parser.add_argument('--svo', type=str, default=None, help='optional svo file, if not passed, use the plugged camera instead')
    parser.add_argument('--img_size', type=int, default=416, help='inference size (pixels)')
    parser.add_argument('--conf_thres', type=float, default=0.4, help='object confidence threshold')
    opt = parser.parse_args()
